...
def extract_resnet_v1_50_mapping_without_logits(resnet_v1_50_variables_mapping):
    """Removes the logits variable mapping from resnet_v1_50_8s to resnet_v1_50 model mapping dict.
    Given the resnet_v1_50_8s to resnet_v1_50 model mapping dict which is returned by
    resnet_v1_50_8s() function, remove the mapping for the fc8 variable. This is done because this
    ...
    resnet_v1_50_variables_mapping : dict {string: variable}
        Dict which maps the resnet_v1_50_8s model's variables to resnet_v1_50 checkpoint variables
        names. Look at resnet_v1_50_8s() function for more details.
...
def resnet_v1_50_8s(image_batch_tensor,
                     number_of_classes,
                     is_training):
    """Returns the resnet_v1_50_8s model definition.
    ...
    adapt_network_for_any_size_input(resnet_v1_50_8s, 8). Note: the upsampling kernel
    ...
    resnet_v1_50_8s_variables_mapping : dict {string: variable}
        Dict which maps the resnet_v1_50_8s model's variables to resnet_v1_50 checkpoint variables
        names. We need this to initilize the weights of resnet_v1_50_8s model with resnet_v1_50 from
        checkpoint file. Look at ipython notebook for examples.
    """
    
    with tf.variable_scope("resnet_v1_50_8s") as resnet_v1_50_8s:
        ...
        resnet_v1_50_8s_variables_mapping = {}

        resnet_v1_50_8s_variables = slim.get_variables(resnet_v1_50_8s)

        for variable in resnet_v1_50_8s_variables:
